  ...
  const updateOrderStatus = (id: number, status: Order['status']) => {
    const updated = orders.map(order =>
      order.id === id ? {
        ...order,
        status,
        ...(status === 'Zakończone' ? { completedAt: new Date().toISOString() } : {})
      } : order
    );
    setOrders(updated);
    saveToStorage('orders', updated);
      ...
      <main className="main">
            ...
            {getUrgentOrders().length > 0 && (
              <div className="alert alert-warning">
                ...
                <div style={{ fontSize: '0.875rem', marginTop: '0.25rem' }}>
                  Zlecenia z terminem w ciągu najbliższych 3 dni
                </div>
              </div>
            )}
            <div className="stats">
          ...
          <div>
            <div className="card">
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
                ...
                <button className="btn" onClick={() => { setModalType('order'); setEditItem(null); setShowModal(true); }}>
                  ➕ Nowe zlecenie
                </button>
              </div>
                              ...
                              <div key={idx} style={{ background: 'white', padding: '1rem', borderRadius: '0.5rem', border: '2px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem', flexWrap: 'wrap' }}>
                                ...
                                <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center', flexWrap: 'wrap' }}>
                                  <select
                                    ...
                                    onChange={(e) => updateOrderStage(order.id, idx, e.target.value, progress.status)}
                                  >
                                  ...
                                  <select
                                    ...
                                    onChange={(e) => updateOrderStage(order.id, idx, progress.assignedTo, e.target.value as StageProgress['status'])}
                                  >
            ...
            {orders.length === 0 && (
              <div className="card">
                <p style={{ textAlign: 'center', color: '#6b7280' }}>Brak zleceń. Dodaj pierwsze zlecenie!</p>
              </div>
            )}
          </div>
        )}
        ...
        {activeTab === 'doctors' && (
          <div className="card">
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ margin: 0 }}>👨‍⚕️ Lekarze</h2>
              <button className="btn" onClick={() => { setModalType('doctor'); setEditItem(null); setShowModal(true); }}>➕ Dodaj</button>
            </div>
            ...
            {doctors.filter(item => !searchTerm || Object.values(item).some(v => String(v).toLowerCase().includes(searchTerm.toLowerCase()))).map(item => (
              <div key={item.id} className="list-item">
                ...
                <div className="list-actions">
                  <button className="btn btn-sm" onClick={() => { setEditItem(item); setModalType('doctor'); setShowModal(true); }}>✏️ Edytuj</button>
                  <button className="btn btn-sm btn-red" onClick={() => setShowConfirmDelete({ type: 'doctor', id: item.id })}>🗑️ Usuń</button>
                </div>
              </div>
            ))}
            {doctors.length === 0 && <p style={{ textAlign: 'center', color: '#6b7280', padding: '2rem' }}>Brak lekarzy. Dodaj pierwszego!</p>}
          </div>
        )}
        ...
        {activeTab === 'invoices' && (
          <div className="card">
            ...
            {invoices.length === 0 && <p style={{ textAlign: 'center', color: '#6b7280', padding: '2rem' }}>Brak faktur. Faktury generują się automatycznie po zakończeniu zleceń.</p>}
          </div>
        )}
      </main>
        ...
        <div className="modal-overlay">
          <div className="modal" style={{ maxWidth: '400px' }}>
            <div className="modal-header">
              <div className="modal-title">⚠️ Potwierdzenie usunięcia</div>
            </div>
            <div className="modal-body">
              <p>Czy na pewno chcesz usunąć ten element? Operacji nie można cofnąć.</p>
            </div>
